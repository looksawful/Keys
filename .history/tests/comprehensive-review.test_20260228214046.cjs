...
test("A14: all default program colors are in colorOptions", () => {
  ...
  for (const [id, prog] of Object.entries(hkData.defaultPrograms)) {
    assert.ok(colorSet.has(prog.color.toLowerCase()), `Program "${id}" color "${prog.color}" not in colorOptions`);
  }
});
...
test("B04: mergePrograms errors on non-object existing", () => {
  const imp = createImporter();
  const res = imp.mergePrograms(null, [
    { id: "a", name: "A", color: "#000", sc: [{ id: 1, a: "x", k: ["X"], d: "easy" }] },
  ]);
...
test("B05: session buildPreviewFromSources aggregates programs from multiple files", () => {
  ...
  const r = hkProgramImportSession.buildPreviewFromSources({
    ...
    sources: [
      {
        source: "a.json",
        text: JSON.stringify({ programs: [{ name: "A", shortcuts: [{ action: "x", keys: "X" }] }] }),
      },
      {
        source: "b.json",
        text: JSON.stringify({ programs: [{ name: "B", shortcuts: [{ action: "y", keys: "Y" }] }] }),
      },
    ],
  });
  assert.ok(r.ok);
...
test("D02: all renderer modules use UMD pattern for testability", () => {
  const modules = ["data.js", "logic.js", "program-import.js", "program-import-session.js", "background-effects.js"];
...
test("E10: F5 must NOT be used as a hotkey default (Electron refresh conflict)", () => {
  // After BUG-2 fix: nav uses F1-F4+F6, quiz uses F7-F11. F5 is free.
  const hotkeyBlock = appSrc.match(/const HOTKEY_DEFAULTS\s*=\s*\{[\s\S]*?\n\s{4}\};/);
  assert.ok(hotkeyBlock, "HOTKEY_DEFAULTS block must exist in app.js");
  const block = hotkeyBlock[0];
  assert.ok(!block.includes("'F5'"), "F5 must not appear in HOTKEY_DEFAULTS (conflicts with Electron refresh)");

  // Verify new mapping: nav
  assert.ok(block.includes("home: 'F2'"), "nav.home should be F2");
  assert.ok(block.includes("editor: 'F3'"), "nav.editor should be F3");
  assert.ok(block.includes("stats: 'F4'"), "nav.stats should be F4");
  assert.ok(block.includes("settings: 'F6'"), "nav.settings should be F6 (was F5)");
  assert.ok(block.includes("help: 'F1'"), "nav.help should be F1");

  // Verify new mapping: quiz
  assert.ok(block.includes("check: 'F7, Enter'"), "quiz.check should be F7, Enter");
  assert.ok(block.includes("deleteLast: 'F8, Backspace'"), "quiz.deleteLast should be F8, Backspace");
