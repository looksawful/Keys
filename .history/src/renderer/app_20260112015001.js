(function () {
  console.log('App.js loading...', { hkData: window.hkData, hkLogic: window.hkLogic });

  // Проверка что все зависимости загружены
  if (!window.hkData || !window.hkLogic) {
    console.error('Dependencies not loaded! Check script order in index.html');
    alert('Ошибка: Модули не загружены. hkData=' + !!window.hkData + ', hkLogic=' + !!window.hkLogic);
    document.body.innerHTML = '<div style="padding: 40px; font-family: sans-serif;"><h1>Ошибка загрузки</h1><p>Не удалось загрузить необходимые модули. Перезагрузите приложение.</p></div>';
    return;
  }
  ...
  console.log('Dependencies loaded successfully', { programDocs: !!programDocs });
  ...
  function statsH() {
    ...
    <div class="card stats-card" data-tooltip="Правильных ответов за всё время"><div class="stats-value">${st.c}</div><div class="stats-label">✓ Правильно</div></div>
  ...
  function handleKeydown(e) {
    ...
    if (showOnboarding && e.key === 'Escape') {
      completeOnboarding();
      return;
    }
    ...
    // Global app shortcuts (not in quiz mode or modal)
    if (!mdl && !showOnboarding) {
      ...
      // Ctrl+S - Stats (prevent browser save)
      ...
      if (e.key === 'Escape' && view !== 'home') {
        ...
        } else if (view === 'study' || view === 'setup' || view === 'results') {
          go('home');
        }
        return;
      }
      // Enter in setup - start quiz
      if (view === 'setup' && e.key === 'Enter') {
        e.preventDefault();
        startQ();
        return;
      }
      // Enter in quiz (after answer shown) - next question
      if (view === 'quiz' && qz && qz.sh && e.key === 'Enter') {
        e.preventDefault();
        nextQ();
        return;
      }
      // Enter in quiz (before check) - check answer
      if (view === 'quiz' && qz && !qz.sh && qz.k.length > 0 && e.key === 'Enter') {
        e.preventDefault();
        checkQ();
        return;
      }
