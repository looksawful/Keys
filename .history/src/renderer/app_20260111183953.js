(function () {
  ...
  function render() {
    ...
    document.getElementById('nav').innerHTML = `
      ...
      <button class="app-nav-link${
        view === 'editor' ? ' active' : ''
      }" data-action="go" data-view="editor">Редактор</button>
      <button class="app-nav-link${
        view === 'stats' ? ' active' : ''
  ...
  function homeH() {
    ...
    <div class="grid">${Object.entries(progs)
      .map(([id, p]) => {
        ...
        <div class="program-meta"><span class="meta-pill">Клавиш: ${p.sc.length}</span><span class="meta-pill">Прогресс: ${pr}%</span></div>
  ...
  function studyH() {
    ...
    <div class="filter-bar">
      ...
      <button class="filter-btn${
        sfil === 'easy' ? ' active' : ''
      }" data-action="set-filter" data-filter="easy"><span class="difficulty-dot difficulty-easy"></span>Лёгкие</button>
      <button class="filter-btn${
        sfil === 'medium' ? ' active' : ''
      }" data-action="set-filter" data-filter="medium"><span class="difficulty-dot difficulty-medium"></span>Средние</button>
      <button class="filter-btn${
        sfil === 'hard' ? ' active' : ''
      }" data-action="set-filter" data-filter="hard"><span class="difficulty-dot difficulty-hard"></span>Сложные</button>
    </div>
  ...
  function setupH() {
    ...
    <div class="setup-group"><label class="setup-label">Сложность</label><div class="setup-options">
      ...
      }" data-action="set-difficulty" data-difficulty="easy"><span class="difficulty-dot difficulty-easy"></span>Лёгкие</button>
      <button class="option-btn${
        qcfg.d === 'medium' ? ' selected' : ''
      }" data-action="set-difficulty" data-difficulty="medium"><span class="difficulty-dot difficulty-medium"></span>Средние</button>
      <button class="option-btn${
        qcfg.d === 'hard' ? ' selected' : ''
      }" data-action="set-difficulty" data-difficulty="hard"><span class="difficulty-dot difficulty-hard"></span>Сложные</button>
    </div></div>
  ...
  function quizH() {
    ...
    <div class="quiz-actions">${
      qz.sh
        ...
        : `<button class="btn btn-secondary" data-action="reset-answer">Сбросить</button>
  ...
  function resultsH() {
    ...
    return `<div class="results-container"><div class="card results-card"><div class="results-kicker">Результат</div><h2 class="results-title">${title}</h2><div class="results-score">${pct}%</div>
  ...
  function editorH() {
    ...
    <div class="editor-main"><div class="editor-section-title"><div class="editor-program-header">
      ...
      <div style="display:flex;gap:.5rem"><button class="btn btn-secondary btn-sm" data-action="show-program-modal" data-program="${eprog}">Изменить</button><button class="btn btn-danger btn-sm" data-action="delete-program">Удалить</button></div></div>
    ...
    <div class="editor-shortcut-list">${
      ...
      <div class="editor-shortcut-keys">${s.k.join(
        ' + ',
                ...
                s.id
              }">Изм.</button><button class="icon-btn icon-btn-delete" data-action="delete-shortcut" data-shortcut="${
                s.id
              }">Удал.</button></div></div>`,
            )
            .join('')
