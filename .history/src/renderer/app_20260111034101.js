(function () {
  ...
  function editorH() {
    ...
    <div class="elist">${Object.entries(progs)
      .map(
        ([id, pr]) => `<div class="eitem${eprog === id ? ' on' : ''}" data-action="select-program" data-program="${id}">
      <span style="color:${pr.color}">${pr.name}</span><span style="color:var(--text3)">${pr.sc.length}</span></div>`,
      )
      .join('')}</div></div>
    <div class="emain"><div class="stitle"><div style="display:flex;align-items:center;gap:.75rem">
      <div class="picon" style="background:${p.color};width:40px;height:40px">${p.name[0]}</div><span>${
      p.name
    }</span></div>
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin:1.5rem 0 1rem">
      <h3 style="color:var(--text2)">Клавиши (${
        p.sc.length
      })</h3><button class="btn btn1 btns" data-action="show-shortcut-modal">+ Добавить</button></div>
    <div class="sclist">${
      p.sc.length
        ? p.sc
            .map(
              (s) => `<div class="scitem"><div class="scact">${s.a}<br><span class="diff diff-${s.d[0]}">${
                s.d === 'easy' ? 'Лёгкий' : s.d === 'medium' ? 'Средний' : 'Сложный'
              }</span></div>
      <div class="sckeys">${s.k.join(
        ' + ',
      )}</div><div class="scbtns"><button class="ibtn ibtn-e" data-action="show-shortcut-modal" data-shortcut="${
                s.id
              }">✏️</button><button class="ibtn ibtn-d" data-action="delete-shortcut" data-shortcut="${
                s.id
              }">🗑️</button></div></div>`,
            )
            .join('')
        : '<div class="empty">Нет клавиш</div>'
    }</div></div></div>`;
  }
  ...
  function statsH() {
    ...
    ${
      st.h.length
        ? `<div class="hlist">${st.h
            .slice()
            .reverse()
            .slice(0, 20)
            .map((h) => {
              const pct = Math.round((h.c / h.t) * 100);
              return `<div class="hitem"><div><strong>${h.n}</strong><br><small style="color:var(--text3)">${new Date(
                h.dt,
              ).toLocaleDateString('ru')}</small></div>
      <div style="color:${pct >= 70 ? 'var(--green)' : 'var(--red)'};font-weight:600">${h.c}/${
                h.t
              } (${pct}%)</div></div>`;
            })
            .join('')}</div>`
        : '<div class="empty">Нет истории</div>'
    }</div>`;
  }
  ...
  function showPM(edit) {
    ...
    <div class="fg"><label class="fl">Цвет</label><div class="colors">${colorOptions
      .map(
        (c) =>
          `<div class="copt${
            p && p.color === c ? ' sel' : ''
      ...
      .join('')}</div></div>
    <div class="mbtns"><button class="btn btn2" data-action="close-modal">Отмена</button><button class="btn btn1" data-action="save-program" data-program="${editId}">${
      p ? 'Сохранить' : 'Создать'
    }</button></div></div></div>`;
    render();
  }
  ...
  function showSM(edit) {
    ...
    <div class="fg"><label class="fl">Клавиши (через +)</label><input class="fi" id="mk" value="${
      s ? s.k.join(' + ') : ''
    }"></div>
    ...
    <div class="mbtns"><button class="btn btn2" data-action="close-modal">Отмена</button><button class="btn btn1" data-action="save-shortcut" data-shortcut="${editId}">${
