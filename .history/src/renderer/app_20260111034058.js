(function() {
  ...
  function render() {
    ...
    document.getElementById('nav').innerHTML = `
      <button class="app-nav-link${view === 'home' ? ' active' : ''}" data-action="go" data-view="home">🏠 Главная</button>
      <button class="app-nav-link${view === 'editor' ? ' active' : ''}" data-action="go" data-view="editor">✏️ Редактор</button>
      <button class="app-nav-link${view === 'stats' ? ' active' : ''}" data-action="go" data-view="stats">📊 Статистика</button>`;
  ...
  function studyH() {
    ...
    <div class="study-program"><div class="study-program-icon" style="background:${p.color}">${p.name[0]}</div><div class="study-program-info"><h2>${p.name}</h2><p>${p.sc.length} клавиш</p></div></div>
    <div class="filter-bar">
      <button class="filter-btn${sfil === 'all' ? ' active' : ''}" data-action="set-filter" data-filter="all">Все</button>
      <button class="filter-btn${sfil === 'easy' ? ' active' : ''}" data-action="set-filter" data-filter="easy">🟢 Лёгкие</button>
      <button class="filter-btn${sfil === 'medium' ? ' active' : ''}" data-action="set-filter" data-filter="medium">🟡 Средние</button>
      <button class="filter-btn${sfil === 'hard' ? ' active' : ''}" data-action="set-filter" data-filter="hard">🔴 Сложные</button>
    </div>
    <div class="shortcut-list">${list.map((s) => `<div class="shortcut-item"><div class="shortcut-action">${s.a}</div><div class="shortcut-keys">
      ${s.k.map((k) => `<span class="key">${k}</span>`).join('<span class="key-separator">+</span>')}
      <span class="badge badge-${s.d}">${s.d === 'easy' ? 'Лёгкий' : s.d === 'medium' ? 'Средний' : 'Сложный'}</span></div></div>`).join('')}</div></div>`;
  }
  ...
  function setupH() {
    ...
    <div class="setup-group"><label class="setup-label">Сложность</label><div class="setup-options">
      <button class="option-btn${qcfg.d === 'all' ? ' selected' : ''}" data-action="set-difficulty" data-difficulty="all">Все</button>
      <button class="option-btn${qcfg.d === 'easy' ? ' selected' : ''}" data-action="set-difficulty" data-difficulty="easy">🟢 Лёгкие</button>
      <button class="option-btn${qcfg.d === 'medium' ? ' selected' : ''}" data-action="set-difficulty" data-difficulty="medium">🟡 Средние</button>
      <button class="option-btn${qcfg.d === 'hard' ? ' selected' : ''}" data-action="set-difficulty" data-difficulty="hard">🔴 Сложные</button>
    </div></div>
    <div class="setup-group"><label class="setup-label">Вопросов (макс: ${max})</label><div class="setup-options">
      ${counts.map((c) => `<button class="option-btn${qcfg.n === c ? ' selected' : ''}" data-action="set-count" data-count="${c}">${c}</button>`).join('')}
    </div></div>
    <button class="btn btn-primary btn-full" data-action="start-quiz"${max === 0 ? ' disabled' : ''}>🚀 Начать</button></div></div>`;
  }
  ...
  function quizH() {
    ...
    const kh = qz.k.length ? qz.k.map((k) => `<span class="quiz-input-key">${k}</span>`).join('<span class="key-separator">+</span>') : '<span class="quiz-input-placeholder">Нажмите клавиши...</span>';
    const cls = qz.sh ? (qz.ok ? ' correct' : ' incorrect') : '';
    return `<div class="quiz-container"><div class="quiz-progress"><div class="quiz-progress-bar"><div class="quiz-progress-fill" style="width:${pct}%"></div></div><span class="quiz-progress-text">${qz.i + 1}/${qz.qs.length}</span></div>
    <div class="card quiz-card"><div class="quiz-program-name">${progs[sel].name}</div><div class="quiz-question">${q.a}</div>
    <div class="quiz-input${cls}">${kh}</div>
    ${qz.sh ? `<div class="quiz-result${qz.ok ? ' correct' : ' incorrect'}">${qz.ok ? '✓ Правильно!' : '✗ Неправильно'}</div>` : ''}
    ${qz.sh && !qz.ok ? `<div class="quiz-answer">Ответ: <strong>${q.k.join(' + ')}</strong></div>` : ''}
    <div class="quiz-actions">${qz.sh ? `<button class="btn btn-primary" data-action="next-question">${qz.i + 1 >= qz.qs.length ? 'Завершить' : 'Далее →'}</button>` :
      `<button class="btn btn-secondary" data-action="reset-answer">🔄 Сброс</button>
      <button class="btn btn-primary" data-action="check-answer"${qz.k.length === 0 ? ' disabled' : ''}>✓ Проверить</button>
      <button class="btn btn-ghost" data-action="skip-question">Пропустить →</button>`}</div></div></div>`;
  }
  ...
  function resultsH() {
    ...
    <p class="results-program">${res.n}</p><div class="results-stats">
      <div class="results-stat"><div class="results-stat-value success">${res.c}</div><div class="results-stat-label">Правильно</div></div>
      <div class="results-stat"><div class="results-stat-value error">${res.t - res.c}</div><div class="results-stat-label">Ошибок</div></div>
      <div class="results-stat"><div class="results-stat-value">${res.t}</div><div class="results-stat-label">Всего</div></div>
